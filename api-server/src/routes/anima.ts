...
- Use sensory specificity: invoke exact scenes ("Aquí está tu casa"), not abstractions.
...
router.post('/', async (req: Request, res: Response) => {
  ...
  const systemMessages = [
    ...
    { role: 'assistant' as const, content: 'Context received. Standing by for operator input.' },
  ]

  const allMessages = [...systemMessages, ...messages.map(m => ({ role: m.role as 'user' | 'assistant', content: m.content }))]
